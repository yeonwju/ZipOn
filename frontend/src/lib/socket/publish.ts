...
import { ChatMessageRequest, LiveChatMessageRequest } from './types'
...
export async function sendLiveChat(
  ...
  // LiveChatMessageRequestDto 형식으로 변환 (content만 전송)
  const requestPayload = {
    content: payload.content,
  }
  ...
  try {
    currentClient.publish({
      destination: `/pub/live/${liveSeq}`,
      body: JSON.stringify(requestPayload),
      headers,
    })

    console.log(`📤 라이브 채팅 메시지 전송 성공: /pub/live/${liveSeq}`, requestPayload)
  } catch (error) {
    console.error('❌ 라이브 채팅 메시지 전송 실패:', error)
  }
}
