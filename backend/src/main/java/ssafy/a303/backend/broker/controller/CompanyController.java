...
public class CompanyController {
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "운영 중인 사업장", content = @Content()),
            @ApiResponse(responseCode = "400", description = "요청 값이 잘못되었거나 형식 오류", content = @Content()),
            @ApiResponse(responseCode = "404", description = "운영 중인 사업장이 아님 또는 존재하지 않음", content = @Content()),
            @ApiResponse(responseCode = "429", description = "외부 API 일일 호출 제한 초과", content = @Content()),
            @ApiResponse(responseCode = "502", description = "외부 API 연동 오류", content = @Content())
    })
    @PostMapping("/status")
